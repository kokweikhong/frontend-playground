...
const DataTable: React.FC = () => {
  const { expensesRecords, setExpensesRecords, setFormType, setExpensesRecord, setOpenForm } = useFinanceExpensesContext()
  ...
  const columns = [
    columnHelper.display({
      ...
      cell: info => (
        <Button
          onClick={() => {
            setExpensesRecord(info.row.original)
            setFormType("update")
            setOpenForm(true)
          }}
          variant="ghost">
          Edit {info.row.original.id}
        </Button>
      )
    }),
  ...
  React.useEffect(() => {
    setExpensesRecords(getFinanceExpensesRecords())
  }, [])
